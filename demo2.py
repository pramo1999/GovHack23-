...
img_list = [util.random_noise(filters.gaussian(im, 1.1), var=5e-4)
...
for i in range(len(image_paths)):
    # img_list.append(cv2.imread(image_paths[i]))
    img_list.append(cv2.cvtColor(cv2.imread(image_paths[i]), cv2.COLOR_BGR2GRAY))
    # gray = cv2.cvtColor(cv2.imread(image_paths[i]), cv2.COLOR_BGR2GRAY)
