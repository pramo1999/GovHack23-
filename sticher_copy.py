...
class Stitcher:
    ...
    def stitch(self, img_names):
        import time
        t0=time.time()
        print(f'{time.time()-t0:.4f}, 1:'); t0=time.time()
        self.initialize_registration(img_names)
        print(f'{time.time()-t0:.4f}, resize_medium_resolution:'); t0=time.time()
        imgs = self.resize_medium_resolution()
        print(f'{time.time()-t0:.4f}, find_features:'); t0=time.time()
        features = self.find_features(imgs)
        print(f'{time.time()-t0:.4f}, match_features:'); t0=time.time()
        matches = self.match_features(features)
        # import pdb; pdb.set_trace()
        import cv2
        res = FeatureMatcher.draw_matches_matrix(imgs, features, matches)
        for i, out in enumerate(res):
            cv2.imwrite(f'out_{i}.jpg', (out[-1]))
        print(f'{time.time()-t0:.4f}, subset:'); t0=time.time()
        imgs, features, matches = self.subset(imgs, features, matches)
        print(f'{time.time()-t0:.4f}, estimate_camera_parameters:'); t0=time.time()
        cameras = self.estimate_camera_parameters(features, matches)
        print(f'{time.time()-t0:.4f}, refine_camera_parameters:'); t0=time.time()
        cameras = self.refine_camera_parameters(features, matches, cameras)
        print(f'{time.time()-t0:.4f}, perform_wave_correction:'); t0=time.time()
        cameras = self.perform_wave_correction(cameras)
        print(f'{time.time()-t0:.4f}, estimate_scale:'); t0=time.time()
        self.estimate_scale(cameras)
        print(f'{time.time()-t0:.4f}, 2:'); t0=time.time()

        imgs = self.resize_low_resolution(imgs)
        print(f'{time.time()-t0:.4f}, warp_low_resolution:'); t0=time.time()
        imgs, masks, corners, sizes = self.warp_low_resolution(imgs, cameras)
        print(f'{time.time()-t0:.4f}, prepare_cropper:'); t0=time.time()
        self.prepare_cropper(imgs, masks, corners, sizes)
        print(f'{time.time()-t0:.4f}, crop_low_resolution:'); t0=time.time()
        imgs, masks, corners, sizes = self.crop_low_resolution(
            imgs, masks, corners, sizes
        )
        print(f'{time.time()-t0:.4f}, estimate_exposure_errors:'); t0=time.time()
        self.estimate_exposure_errors(corners, imgs, masks)
        print(f'{time.time()-t0:.4f}, find_seam_masks:'); t0=time.time()
        seam_masks = self.find_seam_masks(imgs, corners, masks)
        print(f'{time.time()-t0:.4f}, 3:'); t0=time.time()
        ...
        print(f'{time.time()-t0:.4f}, 4:'); t0=time.time()
        print(f'{time.time()-t0:.4f}, initialize_composition, :'); t0=time.time()

        self.initialize_composition(corners, sizes)
        print(f'{time.time()-t0:.4f}, blend_images, :'); t0=time.time()
        self.blend_images(imgs, seam_masks, corners)
        print(f'{time.time()-t0:.4f}, 5:'); t0=time.time()
